...
function main() {
  ...
  const getQuestion = () => {
    const questionALL = document.querySelector(
  ...
  copy?.addEventListener("click", () => {
    ...
    if (!question && !main.querySelector(".extension-body .error")) {
      ...
      main.querySelector(".extension-body")?.appendChild(error);
      return;
    }
    navigator.clipboard.writeText(
      ` Role You are a teacher who can answer questions.
          Instruction Answer the following question in a short and direct manner
          ${getQuestion() as string}
          `
    );
  });
...
chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
  ...
  if (activeTab.url?.startsWith("https://schools.alefed.com/")) {
